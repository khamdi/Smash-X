...
public class ConditionalAttack extends AbstractAttacks {
	...
	public ConditionalAttack (Animator frames, AttackPhase conditional, AttackPhase onhit, AttackPhase missed) {
	...
	public boolean nextStep (Vector2 position, CommandType command) {
		if (hasNextStep ()) {
			...
			if (phases.get(indexPhase).nextStep(this.rightSide, position, command) == false) {
				if (!hit) {
					indexPhase += 2;
					frames.jumpTo(phases.get(0).totalTime + phases.get(1).totalTime);
				}
				else
					indexPhase++;
				
				if (hasNextStep()) {
					phases.get(indexPhase).nextStep(this.rightSide, position, command);
					return true;
				}
			}
			else
