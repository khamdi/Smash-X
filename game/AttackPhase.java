...
public abstract class AttackPhase {
	 ...
	 */
	float activateStunTrigger () throws IllegalStateException {
		if (activationStart) {
			activationStart = false;
			return totalTime;
		}
		throw new IllegalStateException ("Cannot stun twice for the same attack.");
	}
	 ...
	 */
	abstract boolean nextStep (boolean rightSide, Vector2 position);
	 ...
	 */
	abstract int hitCharacter (boolean rightSide, Character c);
}
